...
function getLatAndLng(bitarray, aisType) {
     ...
     *
     */
    var meta = config_1.getMetaDataForAttributeByReport(aisType);
    var meta_lat = meta.lat;
    var meta_lng = meta.lng;
    var lng = bitsHelper_1.parseIntFromBuffer(bitarray, meta_lng.index, meta_lng.len);
    if (lng & 0x08000000) {
        lng |= 0xf0000000;
    }
    lng = parseFloat(String(lng / 600000));
    var lat = bitsHelper_1.parseIntFromBuffer(bitarray, meta_lat.index, meta_lat.len);
    if (lat & 0x04000000) {
        lat |= 0xf8000000;
    }
...
function fetchSog(bitArray, aisType) {
    ...
    */
    ...
    var meta = config_1.getMetaDataForAttributeByReport(aisType)['sog'];
    var sog = undefined; // sog should be undefined by default
    try {
        var _sog = bitsHelper_1.parseIntFromBuffer(bitArray, meta.index, meta.len);
...
function fetchRateOfTurn(bitArray, aisType) {
     ...
     *
    */
    var meta = config_1.getMetaDataForAttributeByReport(aisType)['rot'];
    var rot = undefined;
    var _rot = bitsHelper_1.parseIntFromBuffer(bitArray, meta.index, meta.len);
...
function fetchCourseOverGround(bitArray, aisType) {
     ...
     */
    var meta = config_1.getMetaDataForAttributeByReport(aisType)['cog'];
    var _cog = bitsHelper_1.parseIntFromBuffer(bitArray, meta.index, meta.len);
    if (_cog === 3600) {
        return undefined;
    }
    var cog = Number(parseFloat(String(0.1 * _cog)).toFixed(config_1.Precision));
    return cog;
}
...
function fetchHeading(bitArray, aisType) {
    ...
    var meta = config_1.getMetaDataForAttributeByReport(aisType)['hdg'];
    var _hdg = bitsHelper_1.parseIntFromBuffer(bitArray, meta.index, meta.len);
    if (_hdg === 511) {
        return undefined;
    }
    var hdg = Number(parseFloat(String(0.1 * _hdg)).toFixed(config_1.Precision));
    return hdg;
}
